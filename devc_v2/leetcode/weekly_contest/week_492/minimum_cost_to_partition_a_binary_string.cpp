...
public:
    ...
    long long minCost(string s, int encCost, int flatCost) {
        ...
        return splitAndFindMinCost(0, n-1, flatCost, encCost);
    }

    long long splitAndFindMinCost(int start, int end, int f, int e) {
        long long len = end-start+1;
        long long X = prefix[end+1]-prefix[start];
        long long wholeCost = (X == 0)? f : (len * X * e);
        if (len % 2 != 0) // odd
            return wholeCost;
        int mid = start + (end-start) / 2;
        long long splitCost = splitAndFindMinCost(start, mid, f, e) + splitAndFindMinCost(mid+1, end, f, e);
        return min(wholeCost, splitCost);
    }
};
